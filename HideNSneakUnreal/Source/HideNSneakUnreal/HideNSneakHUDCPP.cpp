...
AHideNSneakHUDCPP::AHideNSneakHUDCPP()
{

}

void AHideNSneakHUDCPP::PostInitializeComponents()
{
	Super::PostInitializeComponents();

	if (GEngine && GEngine->GameViewport) // make sure our screen is ready for the widget
	{
		SAssignNew(MyUIWidget, SHideNSneakTextChatWidget).OwnerHUD(this); // add the widget and assign it to the var
		GEngine->GameViewport->AddViewportWidgetContent(SNew(SWeakWidget).PossiblyNullContent(MyUIWidget.ToSharedRef()));
	}
}

void AHideNSneakHUDCPP::DrawHUD()
{
	Super::DrawHUD();
	...
	if (MyPC->WasInputKeyJustPressed(EKeys::Enter))
		if (MyUIWidget.IsValid())
			if(MyUIWidget->ChatInput.IsValid())
...
void AHideNSneakHUDCPP::AddMessageBP(const int32 Type, const FString& Username, const FString& Text, const bool Replicate)
{
	...
	if (newmessage.Type > 0)
		if (Replicate)
		{
			AHideNSneakTextChatStateCpp* MyPS = Cast<AHideNSneakTextChatStateCpp>(MyPC->PlayerState);
			if (MyPS)
